"""
ui/dialog_arrange_session_live.py — Studio de co-écriture Claude × Réalisateur (LIVE).

Copie Live de ui/dialog_arrange_session.py, calibrée CONDUCTEUR : la fenêtre
co-écrit le conducteur d'une performance live/mapping (actes, tableaux,
ambiances, musiques/BPM) — jamais au format scénario cinéma (INT./EXT.).
Worker : ArrangeSessionChatConducteurWorker (api/live_screenplay.py).
API publique identique au Cinéma : was_applied() / final_screenplay().

Fenêtre ouverte depuis la page Conducteur quand l'utilisateur clique sur
...
Layout :
  ...
  ├────────────────────────────┬──────────────────────────────────────┤
  │  Analyse  │  Conducteur    │  Dialogue avec Claude               │
  │           │  remanié       │  ┌──────────────────────────────┐   │
  ...
  │           │                │  [Envoyer à Claude ☁]               │
  ├────────────────────────────┴──────────────────────────────────────┤
  │  [✓ Appliquer ce conducteur]       [Copier]        [Fermer]       │
  └───────────────────────────────────────────────────────────────────┘
"""
...
class ArrangeSessionDialog(QDialog):
    ...
    def __init__(
        ...
        self._original    = original_conducteur
    ...
    def _build_ui(self):
        ...
        # Panneau gauche : onglets Analyse / Conducteur remanié
        ...
        self._btn_apply = _btn("✓  Appliquer ce conducteur au projet", "accent", 38)
        ...
        self._btn_copy.setToolTip("Copier le conducteur remanié dans le presse-papier")
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        # Onglet 2 — Conducteur remanié (live preview)
        ...
        self._tabs.addTab(tab2_widget, "✦  Conducteur remanié")
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._chat_hint = QLabel(
            ...
            "« Développe la montée de l'acte 2 », « Coupe les stroboscopes »…"
        )
    ...
    def _show_initial_analysis(self):
        ...
        intro = translate(
            "J'ai analysé votre conducteur et rédigé des suggestions détaillées "
    ...
    def _on_screenplay_ready(self, text: str):
        ...
        # Activer l'onglet conducteur et basculer dessus
    ...
    def _on_screenplay_edited(self):
        """Signale que l'utilisateur a modifié manuellement le conducteur."""
    ...
    def final_screenplay(self) -> str:
        """Retourne le conducteur final (version remaniée ou édition manuelle).
        Nom conservé du Cinéma pour compatibilité avec l'appelant."""
