...
class CastingSelector(QWidget):
    ...
    def __init__(self):
        ...
        # Live : pas de section « Décors » ici (widgets conservés mais masqués pour
        # ne pas casser les références internes ; les décors n'ont pas de sens en Live).
        self._decor_toggle.setVisible(False)
        self._no_decor_ref_cb.setVisible(False)
        decor_content.setVisible(False)
...
class _ContinuityBar(QFrame):
    ...
    def __init__(self):
        ...
        col = QVBoxLayout()
        col.setSpacing(2)
        self._title_lbl = QLabel("Plan précédent")
        ...
        # Doublon avec « Raccord automatique » (RENDU & AUDIO) → masqué. La barre
        # reste purement informative (« Raccord depuis : Plan X » + aperçu).
...
class TabT2V(QScrollArea):
    ...
    # relayé par LiveStudioWidget dans le bandeau fixe SOUS les onglets, toujours
    # visible sans scroller (demande Matthieu 2026-07-16, parité Cinéma).
    ...
    def __init__(self):
        ...
        # (Le titre est porté par l'en-tête repliable — voir _btn_style_toggle.)
        ...
        # ── Façade du bâtiment (mapping) — sous le template de style ──────────
        _bref_sep = QFrame()
        _bref_sep.setFixedHeight(1)
        _bref_sep.setStyleSheet(f"background:{C['border']};")
        _fs_outer.addWidget(_bref_sep)
        _bref_title = QLabel("Façade du bâtiment (mapping)")
        _bref_title.setAlignment(Qt.AlignmentFlag.AlignHCenter)
        _bref_title.setStyleSheet(
            f"color:{C['accent']};font-size:11px;font-weight:700;"
            f"background:transparent;border:none;")
        _fs_outer.addWidget(_bref_title)
        self._bref_row = QHBoxLayout()
        self._bref_row.setSpacing(8)
        ...
        self._seq_mode = "live"
        _seq_row = QHBoxLayout()
        _seq_row.setSpacing(8)
        _seq_lbl = QLabel("Générer depuis :")
        _seq_lbl.setStyleSheet(
            f"color:{C['text_dim']};font-size:11px;font-weight:600;background:transparent;")
        _seq_row.addWidget(_seq_lbl)
        self._btn_seq_live    = self._make_seq_btn("Séquences Live", "live")
        self._btn_seq_mapping = self._make_seq_btn("Séquences Mapping", "mapping")
        _seq_row.addWidget(self._btn_seq_live)
        _seq_row.addWidget(self._btn_seq_mapping)
        _seq_row.addStretch()
        lay.addLayout(_seq_row)

        # ── Storyboard selector (lit la séquence Live/Mapping sélectionnée) ───
        ...
        self._casting.setVisible(False)   # replié par défaut (gagne de la place)
    ...
    def _on_shot_selected_impl(self, shot: dict):
        ...
        if not shot:
            ...
            if hasattr(self, "_dyn_cam_toggle_row"):
                # Jamais de caméra dynamique en Mapping (façade fixe, caméra verrouillée)
                self._dyn_cam_toggle_row.setVisible(getattr(self, "_seq_mode", "live") != "mapping")
    ...
    def _start_preview_translate(self):
        ...
        # quit() est INOPÉRANT. Réassigner self._preview_translate_worker laissait
        # l'ancien thread être collecté par le GC PENDANT qu'il tournait encore
        # → « QThread: Destroyed while thread is still running » → plantage de l'app
        # (typiquement en sélectionnant plusieurs plans coup sur coup).
        # abandon_thread() le garde référencé + bloque ses signaux jusqu'à sa fin.
    ...
    def _update_injection_banner(self, *_):
        ...
        # Façade du bâtiment → image de référence en mode Mapping.
        facade_path = ""
        if getattr(self, "_seq_mode", "live") == "mapping":
            from core.live_building import get_building_ref
            facade_path = get_building_ref()
        facade_active = bool(facade_path and os.path.isfile(facade_path))
        ...
        if ref_imgs or style_ref_active or facade_active:
            ...
            if facade_active:
                slots.append(f"Façade : {os.path.basename(facade_path)}")
        ...
        if facade_active:
            lines.append(
                "🌙 Mapping imposé : nuit + façade éclairée par la projection + "
                "fond NOIR PUR (#000000) — rendu projetable en réel.")
    ...
    def start_generation(self):
        ...
        # En MAPPING : aucun préfixe de cadrage (cadre verrouillé, pas de valeur de plan).
        if self._active_shot and getattr(self, "_seq_mode", "live") != "mapping":
            _focal = self._active_shot.get("focal", "")
            if _focal:
                from core.camera_data import focal_to_framing_prefix
        ...
        # Mode Mapping : la façade du bâtiment devient une image de référence
        # → Seedance génère la vidéo SUR la façade (comme les moods).
        if _is_seedance and getattr(self, "_seq_mode", "live") == "mapping":
            from core.live_building import get_building_ref
        ...
        if getattr(self, "_seq_mode", "live") == "mapping":
            _mapping_dna = (
                ...
                "shot (fixed wide shot); the building's architecture itself NEVER rescales "
    ...
    def _make_seq_btn(self, label: str, mode: str) -> QPushButton:
        b = QPushButton(label)
        b.setCheckable(True)
        b.setFixedHeight(30)
        b.setCursor(Qt.CursorShape.PointingHandCursor)
        b.clicked.connect(lambda checked=False, m=mode: self._set_seq_mode(m))
        return b

    def _apply_seq_style(self):
        _acc = C.get("accent", "#7c6bff")
        for b, m in ((self._btn_seq_live, "live"), (self._btn_seq_mapping, "mapping")):
            active = (self._seq_mode == m)
            b.setChecked(active)
            b.setStyleSheet(
                f"QPushButton{{background:{'rgba(78,205,196,0.16)' if active else 'transparent'};"
                f"color:{_acc if active else C['text_secondary']};"
                f"border:1px solid {_acc if active else C['border']};border-radius:7px;"
    ...
    def _set_seq_mode(self, mode: str):
        ...
        if self._seq_mode == "mapping" and getattr(self, "_raccord_auto_cb", None):
            self._raccord_auto_cb.setChecked(True)
        # Mapping = caméra fixe → on retire l'option « Caméra dynamique ».
        if getattr(self, "_dyn_cam_toggle_row", None):
    ...
    def _refresh_bref(self):
        ...
        if path and os.path.isfile(path):
            ...
            pix = QPixmap(path)
            if not pix.isNull():
                thumb.setPixmap(pix.scaled(64, 40, Qt.AspectRatioMode.KeepAspectRatioByExpanding,
                                           Qt.TransformationMode.SmoothTransformation))
            self._bref_row.addWidget(thumb)
            _btn_chg = QPushButton("Remplacer")
            _btn_rm  = QPushButton("✕")
            for _b in (_btn_chg, _btn_rm):
                _b.setFixedHeight(26)
                _b.setCursor(Qt.CursorShape.PointingHandCursor)
            _btn_chg.setStyleSheet(
                f"QPushButton{{background:transparent;color:{C['text_secondary']};"
                f"border:1px solid {C['border']};border-radius:6px;font-size:10px;"
                f"font-weight:600;padding:0 10px;}}"
                f"QPushButton:hover{{color:{C['text_primary']};border-color:{C['border_bright']};}}")
            _btn_rm.setFixedWidth(26)
            _btn_rm.setStyleSheet(
                f"QPushButton{{background:transparent;color:{C['text_dim']};"
                f"border:1px solid {C['border']};border-radius:6px;font-size:10px;font-weight:700;}}"
                f"QPushButton:hover{{color:#fff;background:{C['red']};border-color:{C['red']};}}")
            _btn_chg.clicked.connect(self._on_pick_bref)
            _btn_rm.clicked.connect(self._on_clear_bref)
            self._bref_row.addWidget(_btn_chg)
            self._bref_row.addWidget(_btn_rm)
        else:
            _btn_add = QPushButton("▦  Choisir la façade")
            _btn_add.setFixedHeight(28)
            _btn_add.setCursor(Qt.CursorShape.PointingHandCursor)
            _btn_add.setStyleSheet(
                f"QPushButton{{background:transparent;color:{C['text_dim']};"
                f"border:1px dashed {C['border_bright']};border-radius:6px;"
                f"font-size:10px;font-weight:600;padding:0 12px;}}"
                f"QPushButton:hover{{color:{C['accent']};border-color:{C['accent']};}}")
            _btn_add.clicked.connect(self._on_pick_bref)
            self._bref_row.addWidget(_btn_add)
        self._bref_row.addStretch()

    def _on_pick_bref(self):
        from PyQt6.QtWidgets import QFileDialog
        path, _ = QFileDialog.getOpenFileName(
            self, "Choisir la façade du bâtiment", "",
