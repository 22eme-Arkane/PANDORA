...
from PyQt6.QtWidgets import (
    ...
    QLineEdit, QTextEdit, QComboBox, QProgressBar, QFileDialog, QSpinBox,
...
from api.nano_banana import (
    OptimizeHMCPromptWorker, OptimizeHMCWithReferencesWorker,
...
class HMCDialog(QDialog):
    ...
    def __init__(self, parent=None, item: dict | None = None):
        ...
        self.setWindowTitle("Créer un élément HMC" if not item else "Modifier l'élément HMC")
    ...
    def _build_form(self):
        ...
        title = QLabel("Nouvel élément HMC" if not self._item else "Modifier l'élément HMC")
        title.setStyleSheet(
        ...
        col_name.addWidget(_lbl("Nom"))
        self._name = _input("Ex: Costume victorien homme")
        self._name.setText(self._item.get("name", ""))
        col_name.addWidget(self._name)

        col_type = QVBoxLayout()
        col_type.setSpacing(4)
        col_type.addWidget(_lbl("Type"))
        self._type = QComboBox()
        self._type.addItems(TYPES)
        self._type.setCurrentText(self._item.get("hmc_type", TYPES[0]))
        ...
        col_type.addWidget(self._type)

        row.addLayout(col_name, 2)
        row.addLayout(col_type, 1)
        lay.addLayout(row)
        ...
        self._prompt_lbl = _lbl("Prompt pour Nano Banana")
        ph.addWidget(self._prompt_lbl)
        ph.addStretch()

        self._btn_cloud = QPushButton()
        self._btn_cloud.setFixedSize(26, 26)
        self._btn_cloud.setToolTip("Optimiser avec Claude\nAdapté aux éléments HMC de production cinéma.")
        ...
        self._ref_usage_combo.addItem("🎯  Fidélité exacte  —  reproduit le look précisément", "fidelity")
        ...
        saved_key = self._item.get("hmc_style_key", "") or _style_mod.get_style_key()
        if saved_key:
        ...
        price_lbl = QLabel("💰  Nano Banana (fal.ai) : ~0,03–0,05 $ / image HMC")
        ...
        # personnages et les accessoires (demande utilisateur 2026-07-16). À l'import, on PROPOSE
        # de supprimer le fond (BiRefNet).
        ...
        self._btn_import_photo.setToolTip(translate("Utiliser une photo existante (costume, maquillage, coiffure) plutôt que de générer"))
    ...
    def _build_preview(self):
        ...
        lay.addSpacing(8)
        lay.addWidget(_lbl("Assigner à des personnages", color=CP["text_secondary"]))

        btn_assign = QPushButton("Choisir des personnages…")
        btn_assign.setFixedHeight(36)
        btn_assign.setMinimumWidth(200)
        btn_assign.setStyleSheet(
            f"QPushButton{{background:{CP['bg3']};color:{CP['text_secondary']};"
            f"border:1px solid {CP['border']};border-radius:8px;font-size:11px;"
        ...
        self._assign_lbl = QLabel("Aucun personnage assigné")
        self._assign_lbl.setWordWrap(True)
        self._assign_lbl.setStyleSheet(
            f"color:{CP['text_dim']};font-size:10px;font-family:'Consolas',monospace;background:transparent;"
        )
        lay.addWidget(self._assign_lbl)
    ...
    def _on_type_changed(self, hmc_type: str):
        ...
        placeholders = {
            ...
            "Coiffure":  "Ex: Slicked-back dark hair, period-accurate 1920s style, pomade…\n\nClique sur ☁ pour optimiser via Claude.",
        }
        self._prompt.setPlaceholderText(placeholders.get(hmc_type, ""))
    ...
    def _on_ref_usage_changed(self):
        ...
        if hasattr(self, "_refs_hint_lbl"):
            hints = {
                ...
                "fidelity":    "1re image → look décrit pour reproduction fidèle",
            }
    ...
    def _on_optimize(self):
        ...
        if not text:
            self._status.setText("Décris l'élément avant d'optimiser.")
            return
    ...
    def _on_add_refs(self):
        ...
        if load_config().get("anthropic_key", "").strip():
            ...
            self._worker_refs = OptimizeHMCWithReferencesWorker(
            ...
            self._status.setText(
                f"{len(self._ref_paths)} référence(s) ajoutée(s). Configurez la clé Anthropic."
            )
    ...
    def _import_photo(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, translate("Importer une photo HMC"), "",
    ...
    def _on_generate(self):
        ...
        name = self._name.text().strip() or "hmc"
    ...
    def _on_assign(self):
        from ui.dialog_assign import AssignCharactersDialog
        dlg = AssignCharactersDialog(self, current_ids=self._assigned)
        if dlg.exec() == AssignCharactersDialog.DialogCode.Accepted:
            self._assigned = dlg.get_selected_ids()
            self._refresh_assign_lbl()

    def _refresh_assign_lbl(self):
        import core.casting as casting_api
        if not self._assigned:
            self._assign_lbl.setText("Aucun personnage assigné")
            return
        names = []
        for cid in self._assigned:
            char = casting_api.get_character(cid)
            if char:
                names.append(char.get("name", cid))
        self._assign_lbl.setText(", ".join(names) if names else "Aucun personnage assigné")
    ...
    def _on_save(self):
        ...
        if not name:
            QMessageBox.warning(self, "Nom manquant", "Entre le nom de l'élément HMC.")
            return
        ...
        # Sync bidirectionnel
        import core.casting as casting_api
        item_id = self._saved_data["id"]
        for char in casting_api.list_characters():
            cid = char.get("id", "")
            if cid in self._assigned:
                ...
                if item_id in char.get("hmc_ids", []):
                    char["hmc_ids"] = [h for h in char["hmc_ids"] if h != item_id]
                    casting_api.save_character(char)
