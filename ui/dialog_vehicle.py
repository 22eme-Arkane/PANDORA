...
from api.nano_banana import (
    OptimizeVehiclePromptWorker, OptimizeVehicleWithReferencesWorker,
...
class VehicleDialog(QDialog):
    ...
    def __init__(self, parent=None, item: dict | None = None):
        ...
        self.setWindowTitle("Créer un véhicule" if not item else "Modifier le véhicule")
        self.setStyleSheet(PANDORA_STYLESHEET + f"QDialog{{background:{CP['bg1']};}}")
        from ui.widgets import fit_dialog_to_screen
        fit_dialog_to_screen(self, 0.68, 0.82, 740, 460)
    ...
    def _build_form(self):
        ...
        title = QLabel("Nouveau véhicule" if not self._item else "Modifier le véhicule")
        title.setStyleSheet(
        ...
        self._name = _input("Ex: La Citroën DS noire de Viktor")
        ...
        warn = QLabel(
            ...
            "Certains sujets peuvent être refusés ou modifiés."
        )
        ...
        self._prompt.setPlaceholderText(
            "Ex: Black 1960s Citroën DS, sleek bodywork, chrome trim, period-accurate…\n\n"
        ...
        self._ref_usage_combo.addItem("🎯  Fidélité exacte  —  reproduit le véhicule précisément", "fidelity")
    ...
    def _build_preview(self):
        ...
        self._nav_prev.setFixedSize(52, 28)
        self._nav_prev.setStyleSheet(_nav_ss)
        self._nav_prev.setCursor(Qt.CursorShape.PointingHandCursor)
        self._nav_prev.clicked.connect(lambda: self._navigate_preview(-1))
        ...
        self._nav_next.setFixedSize(52, 28)
        self._nav_next.setStyleSheet(_nav_ss)
        self._nav_next.setCursor(Qt.CursorShape.PointingHandCursor)
        self._nav_next.clicked.connect(lambda: self._navigate_preview(1))
    ...
    def _on_ref_usage_changed(self):
        ...
        if hasattr(self, "_refs_hint_lbl"):
            hints = {
                ...
                "fidelity":    "1re image → véhicule décrit pour reproduction fidèle",
            }
    ...
    def _on_optimize(self):
        ...
        if not text:
            self._status.setText("Décris le véhicule avant d'optimiser.")
            return
    ...
    def _on_add_refs(self):
        ...
        if load_config().get("anthropic_key", "").strip():
            ...
            self._worker_refs = OptimizeVehicleWithReferencesWorker(
    ...
    def _import_photo(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, translate("Importer une photo de véhicule"), "",
    ...
    def _on_generate(self):
        ...
        name = self._name.text().strip() or "vehicle"
        ...
        self._worker_gen = GenerateItemWorker(
            full_prompt, name, subdir="vehicles",
    ...
    def _navigate_preview(self, delta: int):
        if not self._generated_images:
            return
        self._preview_idx = (self._preview_idx + delta) % len(self._generated_images)
    ...
    def _refresh_preview_nav(self):
        n = len(self._generated_images)
        has_many = n > 1
        self._nav_prev.setVisible(has_many)
        self._nav_next.setVisible(has_many)
        self._nav_counter.setVisible(has_many)
        if has_many:
    ...
    def _show_fullsize(self):
        ...
        try:
            ...
            dlg.setWindowTitle("Aperçu véhicule")
            dlg.exec()
    ...
    def _load_preview(self, path):
        ...
        if not pix.isNull():
            pix = pix.scaled(240, 240, Qt.AspectRatioMode.KeepAspectRatioByExpanding,
                             Qt.TransformationMode.SmoothTransformation)
            if pix.width() >= 240 and pix.height() >= 240:
    ...
    def _on_save(self):
        ...
        if not name:
            QMessageBox.warning(self, "Nom manquant", "Entre le nom du véhicule.")
            return
