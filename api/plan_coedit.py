...
def _plan_coedit_system(edition: str, mode: str = "live", discuss_only: bool = False) -> str:
    ...
    if discuss_only:
        ...
        return (
            _intro +
            "Tu DISCUTES avec le réalisateur À PROPOS de ce plan : conseils, idées, "
            "propositions de formulation, questions, retours critiques. Tu NE réécris "
            "PAS le plan et tu ne renvoies AUCUN bloc de plan formaté ni marqueur — "
...
class PlanCoEditWorker(QThread):
    ...
    def run(self):
        try:
            ...
            system = _plan_coedit_system(self._edition, self._mode, discuss_only=self._discuss)
            if _facade_ok:
                system = system + (
